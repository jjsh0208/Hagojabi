(function () {
    ...
    selects.forEach(({ boxId, optionsId, isSingle }) => {
        ...
        function updateDisplay() {
            ...
            selectBox.querySelector('.placeholder').style.display = isSingle
                ? selectedItem
                    ? 'none'
                    : 'block'
                : selectedItem.size > 0
        ...
        optionsBox.querySelectorAll('.option').forEach(option => {
            option.addEventListener('click', e => {
                ...
                if (isSingle) {
                    ...
                    selectedItem.has(selectedText)
                        ? selectedItem.delete(selectedText)
                        : selectedItem.add(selectedText);
                }
    ...
    document.addEventListener('click', event => {
        selects.forEach(({ boxId, optionsId }) => {
            ...
            if (selectBox && optionsBox) {
                if (
                    !selectBox.contains(event.target) &&
                    !optionsBox.contains(event.target)
                ) {
                    if (selectBox.getAttribute('aria-expanded') === 'true') {
                        selectBox.setAttribute('aria-expanded', 'false');
                        optionsBox.style.display = 'none';
                    }
                }
